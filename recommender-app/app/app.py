...
def get_user_embedding(uid):
    ...
    request.inputs['string_lookup_1_input'].CopyFrom(
            tf.make_tensor_proto(uid, shape=[1]))
...
class Recommender(Resource):
    def get(self, uid):
        user_recs = {"uid": [], "recommendations": []}
        user = tf.convert_to_tensor(uid, dtype="string")
        ...
        user_recs["uid"].append(user.numpy().decode("utf-8"))
        user_recs["recommendations"].append(candidates)
        return user_recs

api.add_resource(Recommender, '/recommend/<uid>')
